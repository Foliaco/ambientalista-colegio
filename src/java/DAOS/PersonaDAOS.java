...
public class PersonaDAOS implements Interfaz_Persona {
    ...
    public ArrayList<PersonaDTO> getOnePerson(Integer id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                person.setCedula(rs.getNString("cedula"));
    ...
    public boolean updatePerson(PersonaDTO person) {
        ...
        try {
            ...
            sql = "UPDATE personas SET cedula='" + person.getCedula() + "',name= '" + person.getName() + "', middlename='" + person.getMiddlename() + "',"
                    + "lastname='" + person.getLastname() + "',seccondlastname='" + person.getSecondlasname() + "',"
                    + "telefono='" + person.getTelefono() + "', email='" + person.getEmail() + "' WHERE id_persona= '" + person.getId_persona() + "'";
            System.out.println(sql);
            conex = DB.conect();
            stm = conex.createStatement();
            valor = stm.executeUpdate(sql);
    ...
    public boolean eliminarPerson(Integer id_user) {
        int valor = 0;
        try {
            sql = "delete from personas where id_persona = "+id_user+"";
            conex = DB.conect();
            stm = conex.createStatement();
            valor = stm.executeUpdate(sql);
